...
class LengthReversal(BuildableDataset, ComputableBenchmark):
    """
    Benchmark for the length-reversal task, where the length of each word in a list is recorded and then the order of the resulting lengths is reversed.
    ...
    _INSTRUCTIONS_TEMPLATE = "Substitute each word in the list [{}] for its character length, and then reverse the order of the resulting list."

    @dataclass(kw_only=True)
    class Config(Benchmark.Config):
        """The configuration of the length-reversal benchmark."""

        name: str = "LengthReversal"
        """The name of the benchmark."""

    @property
    def config_cls(self) -> type[Config]:
        return LengthReversal.Config

    def __init__(self, config: Config):
        """
        Initialize the length-reversal benchmark.
        ...
        self._config: LengthReversal.Config

    @classmethod
    def compute_target(cls, input: str) -> str:
        return " ".join([str(len(w)) for w in input.split(" ")][::-1])
    ...
    def build(
        ...
        """
        Build the length-reversal benchmark.
